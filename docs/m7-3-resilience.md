...
| 超限文件 | 1 GiB + 1 字节的 sparse bundle | 读取前 | 稳定拒绝，不分配完整输入/解析 JSON |
...
这是固定压力负载的验收，不能据此保证接近容量上限的 bundle 在任意内存配置上
不会 OOM。Bundle 文件读写、HTTP 回读和 canonical hash 已改为流式处理，保留
单会话 transfer 384 MiB、bundle/计划 1 GiB 上限；映射、schema 校验和对账仍会
保留结构化对象，尚未采用 sidecar。RSS 包含堆外内存，允许超过 V8 堆限制。

## 本机结果

macOS / Node 18.20.8，三组通过；完整质量门禁 403 项通过。

| 场景 | worker RSS 采样最大值 | 实际导入 | 重复导入 | 退出信号 |
| --- | ---: | ---: | ---: | --- |
| 大会话 | 523,419,648 bytes（约 499 MiB） | 1 | 0 | 正常 |
| 导入前中断后恢复 | 217,104,384 bytes | 12 | 0 | SIGKILL 后恢复 |
| 导入后中断后恢复 | 216,236,032 bytes | 12 | 0 | SIGKILL 后恢复 |
