...
本手册说明如何把一个 TRAE 会话迁移到 OpenCode。按顺序完成即可，正常流程只需要运行
一条命令和输入两个列表编号：
...
没有持久化某些字段，但其余可验证内容仍可能安全迁移。

## 迁移过程中程序会做什么

选择完成后无需继续输入内容。程序会依次执行以下操作：

1. 从所选 workbench 的 renderer 导出会话。
2. 检查 bundle 是否超过 `128 MiB`。
...
1. 会话标题与来源相符。
2. 消息顺序和最近一轮内容正确。
3. assistant 执行任务时已持久化的进度描述与最终输出均可见，且顺序正确。
4. 工具记录没有被显示为大段 JSON 聊天正文。
5. 完整性存在缺口时，界面显示的是明确缺失提示，而不是伪造的回答或输出。
...
若 OpenCode 中已存在完全一致的目标会话，会显示“目标会话已存在，未重复导入”。
