...
如果 service descriptor 指向正在运行的其他版本，程序会立即停止，不会并发启动
`2.0.12` 访问同一数据库。使用 OpenCode 桌面端 `2.0.16` 时，请在迁移前完全退出
桌面端，迁移完成后再打开。
...
1. 会话标题与来源相符。
2. 消息顺序和最近一轮内容正确。
3. assistant 执行任务时已持久化的进度描述与最终输出均可见，且最终正文前有水平分隔线。
...
先持久化新端点再续跑。没有成功会话可作为证据、目标缺失或内容已修改时不会重绑定。
...
### 工具提示检测到 OpenCode 2.0.16

当前迁移契约只验证了 OpenCode `2.0.12`。完全退出 OpenCode 桌面端，确认其后台服务
也已结束，再重新运行 `npm run migrate:local`。不要让 `2.0.16` 桌面端与迁移程序启动
的 `2.0.12` 服务同时访问同一个 `opencode.db`。
