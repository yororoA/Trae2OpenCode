    ...
    : message.type === "assistant" && Array.isArray(message.content)
      ? message.content.flatMap((block) =>
        isRecord(block) && block.type === "text" && typeof block.text === "string"
          ? [`[Assistant]\n${block.text}`] : []).join("\n\n")
      : "";
  if (!text) return tailUtf8(previous, MAX_CONTINUATION_SUMMARY_BYTES);
  const textTail = tailUtf8(text, MAX_CONTINUATION_SUMMARY_BYTES);
  if (Buffer.byteLength(textTail, "utf8") >= MAX_CONTINUATION_SUMMARY_BYTES) return textTail;
  return tailUtf8(
    previous ? `${previous}\n\n${textTail}` : textTail,
