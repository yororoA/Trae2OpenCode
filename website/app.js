const translations = {
  zh: {
    ...
    "compatibility.note": "表中 OpenCode 版本为已验证基线；其他稳定 1.x/2.x 需使用同版本 CLI 并通过隔离验证。发现两个方言时可在一次运行中同时迁移。",
  ...
  en: {
    ...
    "compatibility.note": "Listed OpenCode versions are verified baselines. Other stable 1.x/2.x releases need a matching CLI and isolated verification. When both dialects are found, one run can migrate to both.",
